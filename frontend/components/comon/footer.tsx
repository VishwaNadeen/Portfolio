...
export default function Footer() {
  ...
  return (
    <footer className="relative overflow-hidden bg-gradient-to-r from-[#020817] via-[#0f172a] to-[#020817]">
      {/* glowing top border */}
      <div className="absolute left-0 right-0 top-0 h-px animate-pulse bg-gradient-to-r from-transparent via-cyan-400/50 to-transparent" />
      ...
      <div ref={ref} className="w-full px-4 py-6 sm:px-6 lg:px-10">
        <div
          className={[
            "flex flex-col items-center justify-between gap-5 text-center sm:gap-4 md:flex-row md:text-left",
            visible
              ? "animate-[fadeUp_0.3s_cubic-bezier(.22,.8,.5,1)_0.05s_both]"
              : "opacity-0",
          ].join(" ")}
        >
          {/* Copyright */}
          <span className="text-sm leading-6 text-slate-600 md:shrink-0">
            © {new Date().getFullYear()}{" "}
            <span className="animate-[shimmer_4s_ease-in-out_infinite] bg-gradient-to-r from-slate-200 via-cyan-400 to-indigo-400 bg-[length:250%_auto] bg-clip-text font-semibold text-transparent">
              Vishwa Nadeen
            </span>
          ...
          <div
            className={[
              "flex flex-wrap items-center justify-center gap-3 sm:gap-4 md:justify-end",
              visible
                ? "animate-[fadeUp_0.3s_cubic-bezier(.22,.8,.5,1)_0.08s_both]"
                : "opacity-0",
            ].join(" ")}
          >
            {SOCIALS.map((s, i) => (
              <a
                ...
                title={s.label}
                className={[
                  "inline-flex h-[38px] w-[38px] items-center justify-center rounded-full sm:h-[40px] sm:w-[40px]",
                ...
                style={{
                  ...
                  transitionProperty:
                    "opacity, transform, color, box-shadow, background-color",
                  transitionDuration: visible
                    ? `0.25s, 0.25s, 0.1s, 0.1s, 0.1s`
                    : "0.25s",
